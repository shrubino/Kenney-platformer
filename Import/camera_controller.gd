...
@onready var _over_shoulder_pivot: Node3D = $OverShoulderPivot
@onready var _camera_spring_arm: SpringArm3D = $SpringArm3D
@onready var _third_person_pivot: Node3D = $SpringArm3D/Marker3D
...
func _process(delta: float) -> void:
	...
	#turning these off for now bc no inputs for camera controls
